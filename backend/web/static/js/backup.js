...
const Backup = {
  ...
  async render(container) {
    container.innerHTML = `
            ...
            <div class="mb-8" id="backup-job-status"></div>
            <div class="flex-between mb-8">
                <span class="card-title" style="margin:0">wal-g backups</span>
                <div style="display:flex;gap:6px">
                    <button class="btn btn-sm" id="btn-verify-latest" title="Fetch latest backup into scratch storage and validate its core files">
                        <i data-lucide="shield-check" class="icon-sm"></i> verify latest
                    </button>
            ...
            <div class="card" style="padding:0">
                <div class="overflow-auto" style="height:calc(100vh - 192px); max-height: none;">
                    <table class="data-table" style="table-layout:fixed;width:100%">
                        <thead><tr><th>name</th><th style="width:117px">size</th><th style="width:90px">modified</th><th style="width:60px">method</th><th>wal</th><th style="width:117px">actions</th></tr></thead>
    ...
    lucide.createIcons();
    document
      .getElementById("btn-trigger-backup")
      .addEventListener("click", () => this.triggerBackup());
    document
      .getElementById("btn-verify-latest")
      .addEventListener("click", () => this.verifyBackup("LATEST"));
    document
      .getElementById("btn-pitr")
      .addEventListener("click", () => this.showPITRModal());
  ...
  async loadBackups() {
    ...
    try {
      ...
      if (!list.length) {
        tbody.innerHTML =
          '<tr><td colspan="6" class="text-center dim py-16">no backups found</td></tr>';
        return;
      }
          ...
          return `<tr>
                ...
                <td>
                    <div style="display:flex;gap:4px">
                        <button onclick="Backup.verifyBackup('${escHtml(b.backup_name)}')" class="btn btn-sm" title="verify backup fetch" style="font-size:9px">verify</button>
                        <button onclick="Backup.restore('${escHtml(b.backup_name)}')" class="btn btn-sm btn-danger">restore</button>
                    </div>
                </td>
            </tr>`;
        })
  ...
  async verifyBackup(name) {
    try {
      showToast("starting backup verification...", "info");
      const res = await apiProtected("/backups/verify", {
        method: "POST",
        body: JSON.stringify({ backupName: name }),
      });
      this.trackJob(res.data?.jobId);
      showToast("backup verification started", "success");
    } catch (e) {
      /* handled */
    }
  },
  ...
  async restore(name) {
    ...
    try {
      ...
      const res = await apiProtected("/backups/restore", {
        method: "POST",
        body: JSON.stringify({ backupName: name }),
      });
      this.trackJob(res.data?.jobId);
      showToast("restore in progress", "success");
    } catch (e) {
  ...
  showPITRModal() {
    ...
    document.getElementById("pitr-datetime").value = now
      .toISOString()
      .slice(0, 19);
    ...
    document
      .getElementById("pitr-execute")
      .addEventListener("click", () => this._executePITR(cleanup));
  },

  async _executePITR(cleanup) {
    const targetTime = document.getElementById("pitr-datetime")?.value;
    const confirmText = document
      .getElementById("pitr-confirm-input")
      ?.value?.trim();

    if (!targetTime) {
      showToast("select a target date & time", "error");
      return;
    }
    if (confirmText !== "PITR_CONFIRM") {
      showToast("type PITR_CONFIRM to proceed", "error");
      document.getElementById("pitr-confirm-input").style.borderColor =
        "var(--red)";
      return;
    }
    ...
    try {
      ...
      this.trackJob(res.data?.jobId);
      showToast("PITR in progress", "success");
      cleanup();
    } catch (e) {
  ...
  async renderJobStatus() {
    const el = document.getElementById("backup-job-status");
    if (!el) return;
    if (!this._jobId) {
      el.innerHTML = "";
      return;
    ...
    try {
      ...
      el.innerHTML = renderJobSummary(job, {
        showDownload: job?.type === "backup_verify",
      });
      if (job.status === "succeeded") this.loadBackups();
      if (job.status !== "running" && this._poller) {
  ...
  trackJob(jobId) {
    ...
    this._poller = setInterval(() => this.renderJobStatus(), 3000);
  },
};
